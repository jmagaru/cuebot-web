...
class CuebotWebFormatter extends JsonFormatter {
  constructor(options) {
    super(options);
    ...
    this.log = (data) => {
      ...
      JSON.parse(data).forEach((feature) => {
        ...
        detailsSummaryTable.push([
          { colSpan: 9, content: colors.blue(feature.name) },
        ]);
        feature.elements.forEach((scenario) => {
          let stats = scenario.steps.reduce(
            (st, step) => {
              let is_ = (keyword) =>
                step.hidden ? 0 : step.result.status == keyword ? 1 : 0;
          ...
          let _steps = Object.entries(stats).reduce(
            (sum, stat) => sum + stat[1],
            0
          );
          let isPassed = (stats) => {
            return (
              ...
              Object.entries(stats)
                .filter((stat) => stat[0] != "passed")
                .reduce((sum, stat) => sum + stat[1], 0) == 0
            );
          };
          ...
          detailsSummaryTable.push([
            ...
            _steps,
            stats.passed,
            this.condColor((value) => value > 0, stats.failed, {
              true: colors.red,
              false: colors.white,
            }),
            this.condColor((value) => value > 0, stats.skipped, {
              true: colors.cyan,
              false: colors.white,
            }),
            this.condColor((value) => value > 0, stats.pending, {
              true: colors.yellow,
              false: colors.white,
            }),
            this.condColor((value) => value > 0, stats.ambiguous, {
              true: colors.gray,
              false: colors.white,
            }),
            this.condColor((value) => value > 0, stats.undf, {
              true: colors.gray,
              false: colors.white,
            }),
          ]);
        ...
        featuresSummaryTable.push([
          ...
          totals.steps > 0
            ? this.condColor(
                (value) => (value >= 100 ? 0 : value < 50 ? 1 : 2),
                ((totals.passed / totals.steps) * 100).toFixed(2),
                { 0: colors.green, 1: colors.red, 2: colors.yellow }
              )
            : colors.yellow("---"),
    ...
    options.eventBroadcaster.on("test-case-started", ({ sourceLocation }) => {
      const {
        gherkinDocument,
        pickle,
      } = options.eventDataCollector.getTestCaseData(sourceLocation);
      readline.clearLine();
      readline.cursorTo(0);

      const { feature } = gherkinDocument;
      this.logFn(
        `\n${colors.magenta.bold(feature.keyword)}: ${colors.bgBlue(
          feature.name
        )} > ${colors.bold.underline(pickle.name)} \n`
      );
    });
    ...
    options.eventBroadcaster.on("test-step-started", ({ testCase, index }) => {
      ...
      if (pickleStep) {
        if (!this.inCi) {
          this.spinner.text = `\t ${colors.yellow(gherkinKeyword)}${
            pickleStep.text
          }`;
          this.spinner.start();
        }
    ...
    options.eventBroadcaster.on(
      ...
      ({ testCase, index, result }) => {
        ...
        if (pickleStep) {
          let message = `${this.resolveStatus(result.status)} - ${colors.yellow(
            gherkinKeyword
          )}${this.sanitize(pickleStep.text, true)}${this.displayDuration(
            result
          )}`;
          ...
          if (pickleStep.arguments.length > 0) {
            ...
            if (Utils.isJSON(content)) {
              content = JSON.stringify(
                this.sanitizeJson(JSON.parse(content), true),
                null,
                4
              );
              content =
              ...
              pickleStep.arguments[0].rows.forEach((value, index) => {
                if (index > 0) {
                  contentTable.push(
                    value.cells.map((v) =>
                      Store.sanitize(v.value, true).substring(0, 50)
                    )
                  );
                }
              });
    ...
    options.eventBroadcaster.on("test-run-started", () => {
      this.logFn(`${colors.green("Cuebot-web")} Test Run \n`);
    });
  ...
  displayNetworkStatistics() {
    ...
    Object.entries(this.stats()).forEach((e) => {
      networkSummaryTable.push([
        { colSpan: 6, content: colors.blue.bold(e[0]) },
      ]);
      Object.entries(e[1]).forEach((p) => {
        networkSummaryTable.push([
          ...
          prettyTime(p[1].min),
          prettyTime(p[1].max),
          this.condColor((value) => value > 0, p[1].errors.toFixed(1), {
            true: colors.red,
            false: colors.white,
          }),
        ]);
  ...
  displayJsonErrorBlock(errors) {
    ...
    errors.forEach((error) => {
      let jsonResponse = JSON.stringify(
        Store.resolve(`__response-${error.source}`),
        null,
        4
      ).split("\n");
      ...
      message.push(
        "\n\t" +
          colors.red.underline(
            `${error.message}${dataPathMessage}based on ${error.schemaPath}`
          )
      );
      if (error.lineNumber) {
  ...
  displayDuration(result) {
    return process.env.AUTOKIN_TIME == "true" && result.duration
...
module.exports = CuebotWebFormatter;
